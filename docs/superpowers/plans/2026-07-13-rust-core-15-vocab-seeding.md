...
> **Rev 2 (adversarial review):** fixed the reflection-wipes-marker BLOCKER — `reflect()` now carries `current`'s internal (`_`-prefixed) sections into the rebuilt memory (a fifth exclusion site), so `_seeds` survives every reflection and deleted seeds stay dead (Task 1, WE-B gains a survives-a-reflection line); the empty-wipe guard **and** `memory_block` now compare **non-internal** sections only (internal-only memory no longer renders "(empty)" or hard-errors reflection forever); `churn_between` excludes internal sections; `UpdateMemoryTool` now rejects `_`-prefixed section names (Task 1). Task 4: pinned the vocab card to first notes-screen appearance behind an `onboardingVocabCardShown` UserDefaults flag (no demo-walk onboarding step is built by this plan); free-form Done writes are per-term catch-and-continue with a surfaced skipped-count. Renamed the vocab-funnel label **Plan 12 → Plan 10** (code headers + CANON). D4 notes partial-application skips are permanently foreclosed by the marker (deliberate) and `clamp_to_cap` can evict old facts on large seeds.
>
...
- A `Pinned`/protected tier for seeds (if device testing shows erosion) is the **Plan 10 D3 escalation — deferred, measure first** (Open Question 3).
...
- **Partial-application skips are permanently foreclosed by the marker — deliberate.** The marker is written **after** the loop regardless of `skipped_over_budget`/`skipped_full` (WE-D/WE-E), so a term the pack could not add on the first pass is **never retried** by a same-`version` re-seed (it short-circuits on `already_seeded`). This is acceptable because curated packs are ≤~20 terms (D7) — the budget/cap paths are test-only guardrails, not a normal path; a genuinely enlarged pack ships as a `version` bump. Note also that `mark_pack_seeded` runs before `clamp_to_cap(DEFAULT_WORD_CAP)`: on a large seed the 500-word clamp **can** evict pre-existing older facts (by `(source.rank, last_touched, name)`) — the **same** discipline the existing CRUD `add_vocabulary_term` already applies after every add, not a new eviction behavior.

### D5-15. Internal `_`-prefixed sections — the marker never leaks and never evicts

The `_seeds` marker rides the **existing** `Memory.sections` map (no new storage, no migration — section names are free-form strings, exactly as `session_meta` rode the artifact table in Plan 14). To keep it out of the agent context and safe from eviction, a one-line convention: **a section whose name starts with `_` is internal** — excluded from `render`/`to_prompt`, `word_count`, `clamp_to_cap` (never an eviction candidate), and `prune_stale` (never aged out). `vocabulary_terms()` already reads only `VOCABULARY_SECTION`, so it is unaffected. **No existing section is named with a `_` prefix**, so this is behavior-preserving for all current data (asserted by the unchanged existing tests staying green). One helper `fn is_internal_section(name: &str) -> bool { name.starts_with('_') }` gates the sites. **Rev 2 — there are FIVE, not four:** render, word_count, clamp_to_cap, prune_stale — **plus `ReflectionEngine::reflect`**, which rebuilds a fresh `Memory` from only the model-echoed sections and would otherwise drop `_seeds` every reflection (deleted seeds would resurrect). `reflect()` copies `current`'s internal sections into the rebuilt memory before the empty-wipe guard; the guard and `memory_block` are corrected to compare **non-internal** sections; `churn_between` excludes internal. `UpdateMemoryTool` also rejects `_`-prefixed section writes so the agent cannot forge a marker. (All in Task 1.)
...
- **Survives a reflection cycle.** Run one `ReflectionEngine::reflect` over this memory (model rewrites `vocabulary`/`people`, never echoes `_seeds` — it never saw it, D5). The rebuilt memory **carries `_seeds` forward** from `current` (Rev 2 fifth site), so `is_pack_seeded("landscape:1")` stays `true` **after** reflection. Re-invoking `seed_vocabulary("landscape", 1, …)` post-reflection still short-circuits (`already_seeded:true`) and `boxwood` still stays gone. Without the carry-forward, reflection would wipe the marker and the very next re-seed would resurrect every user-deleted term — the Rev 2 BLOCKER. ✓
...
### Task 1 — internal `_`-section convention + seeded-pack marker + reflection carry-over (harness; D4-15/D5-15)

**Why this is bigger than "four sites."** The `_seeds` marker rides `Memory.sections`, but `ReflectionEngine::reflect` (`crates/harness/src/reflection/engine.rs:155–172`) **rebuilds a brand-new `Memory` from only the sections the model echoes back**, and the coordinator (`crates/murmur-core/src/coordinator.rs:128–135`) wholesale-replaces + persists it. With D5's render-skip, the model **never sees** `_seeds` → never echoes it → **every reflection erases the marker** → `is_pack_seeded` goes false → the pack re-applies → **user-deleted seeds resurrect**. So the exclusion is not four sites but **five**: `reflect()` must carry `current`'s internal sections into the rebuilt memory. Two adjacent guards in the same file also mis-handle an internal-only memory and must compare **non-internal** sections.

- [ ] **RED — memory (`crates/harness/src/memory/mod.rs` tests):** (a) a section named `"_seeds"` with an entry is **absent** from `to_prompt()` output; (b) its words are **not** counted by `word_count()`; (c) `clamp_to_cap(tiny_cap)` with an over-cap `vocabulary` section **evicts vocabulary entries but never the `_seeds` entry** (assert the marker survives to zero remaining budget); (d) `prune_stale(now, small_age)` with a very old `_seeds` entry **keeps** it (never aged out); (e) `mark_pack_seeded("landscape:1")` then `is_pack_seeded("landscape:1") == true` and `is_pack_seeded("property:1") == false`; marking twice is idempotent (one entry). Add a **regression pin**: a normal section (`"vocabulary"`, `"people"`) is **still** rendered/counted/evictable exactly as before (guards against over-broad exclusion).
- [ ] **RED — reflection carry-over (`crates/harness/src/reflection/engine.rs` tests, `MockProvider`):** build a `current` memory holding `vocabulary` + `people` **and** a `_seeds` entry `"landscape:1"`. Drive one `reflect()` whose mocked `write_memory` echoes only `vocabulary`/`people` (as a real model would — it never saw `_seeds`). Assert: **(a)** the returned `ReflectionOutcome.memory` still has `is_pack_seeded("landscape:1") == true` (marker preserved through the rebuild); **(b)** the `CompletionRequest` the mock received (capture it in the spy provider) **never contains** `_seeds` or `landscape:1` in its user message (the marker never leaks into the reflection prompt); **(c)** `outcome.churn` **equals** the churn computed over the same reflection **without** any `_seeds` entry present — i.e. the carry-over does not distort the churn signal. Also add: **(d)** an internal-**only** `current` (just `_seeds`, no real sections) whose model echoes `{}` does **not** hard-error the empty-wipe guard, and `memory_block(current)` for that memory does **not** render `"(empty)"` in a way that erases real content — assert the guard only fires when the **non-internal** sections go from non-empty to empty.
- [ ] **RED — `UpdateMemoryTool` (`crates/harness/src/memory/tool.rs` tests):** a `remember` op with `section: "_seeds"` (or any `_`-prefixed section) returns a `HarnessError::Tool` and **does not** mutate or save — the agent tool may not write internal sections.
- [ ] **GREEN — memory:** add `pub(crate) const SEED_MARKER_SECTION: &str = "_seeds";` and `pub(crate) fn is_internal_section(name: &str) -> bool { name.starts_with('_') }`. Gate the **four** memory sites on it: `render` (skip internal), `word_count` (skip internal), `clamp_to_cap` (exclude internal from the candidate iterator), `prune_stale` (skip internal sections in `retain`). Add `pub fn mark_pack_seeded(&mut self, key: &str)` (idempotent `remember_from(SEED_MARKER_SECTION, key, 0, FactSource::Stated, None)` — `now=0` is fine, markers are never aged) and `pub fn is_pack_seeded(&self, key: &str) -> bool` (`section_texts(SEED_MARKER_SECTION).contains(&key)`).
- [ ] **GREEN — reflection (`engine.rs`), the FIFTH site + two guards:**
  - **Carry-over (new):** after the rebuild loop populates `memory` from the model's echoed sections and **before** the empty-wipe guard, copy every internal section from `current` into `memory` verbatim: `for (name, entries) in &current.sections { if is_internal_section(name) { memory.sections.insert(name.clone(), entries.clone()); } }`. (Internal sections are never model-authored, so a straight copy preserves the marker's `last_touched`/`source`; the clamp at `:184` already skips internal after the memory-GREEN change.)
  - **Empty-wipe guard (`:176`), fix:** compare **non-internal** sections only — `let non_internal = |m: &Memory| m.sections.iter().any(|(n, es)| !is_internal_section(n) && !es.is_empty());` and fire the error when `non_internal(current) && !non_internal(&memory)`. This must be evaluated on the **content** sections, not raw `sections.is_empty()` — otherwise the carried-over `_seeds` masks a genuine model wipe (guard never fires) and a legitimate model `{}` over an internal-only memory hard-errors reflection forever.
  - **`memory_block` (`:79–84`), fix:** return `"(empty)"` only when there are **no non-internal, non-empty** sections (reuse the same `non_internal` predicate), so an internal-only memory renders `""` rather than a spurious block, and a memory with real content always renders it.
  - **`churn_between` (`:192`), fix:** exclude internal sections from both key sets so the carry-over never enters the churn numerator/denominator (symmetric today, but pinned so a future asymmetry can't skew the adaptive-reflection signal).
- [ ] **GREEN — `UpdateMemoryTool` (`tool.rs`):** in `execute`, after resolving `section`, reject internal names before mutating: `if is_internal_section(section) { return Err(Self::err(format!("section '{section}' is internal and cannot be written"))); }` (cheap one-liner; keeps `_seeds` a marker-only channel now that it is cap/prune-exempt).
- [ ] **Gate:** `nix develop -c cargo test -p harness` + `nix develop -c cargo clippy -p harness --all-targets -- -D warnings`. (All **pre-existing** memory + reflection tests must stay green — proof the exclusion + carry-over are behavior-preserving for non-`_` sections and for memories with no internal section.)
...
- [ ] **Schema gate** (`VocabPackTests` in the iOS test target — the ruling's "CI schema test"): for **every** bundled pack assert: decodes; `version >= 1`; `terms` non-empty and `count <= 60` (SEED_MAX); no case-insensitive duplicates; each term `1…6` whitespace words (mirrors `MAX_VOCABULARY_TERM_WORDS`), non-blank. **This gate is what makes `Empty`/`TooLong` unreachable in Task 2.** **Drift note:** the `60` and `6` here are **Swift literals with no compile-time cross-pin** to Rust `SEED_MAX`/`MAX_VOCABULARY_TERM_WORDS` (they live in different toolchains). Accepted drift — add **one comment line on each side** (`// keep in sync with ffi SEED_MAX (60)` here; `// keep in sync with VocabPackTests (60)` at the Rust `const SEED_MAX`) so a future bump touches both.
- [ ] **Card wiring** (`// sac:` owns visuals/placement — D9): a minimal reference `VocabSeedCard` presenting the pack terms as default-on deselectable chips + the existing free-form add bar; **Done** calls `seedVocabulary(trade, version, confirmedChips)` then adds each free-form term (per-term catch-and-continue, below); **Skip** writes nothing. **Concrete presentation hook (no demo-walk step exists):** `OnboardingFlow.swift` is `WELCOME → YOUR BUSINESS → MIC → onComplete()` and stops — there is **no onboarding demo-walk construct** (the `TODO(#181)` at `OnboardingFlow.swift:273` was a placeholder for one that was never built, and **this plan does not build it**). To honor the CANON ruling's *intent* ("walk before the vocab card") without inventing an onboarding demo-walk step, present `VocabSeedCard` on the **first appearance of the notes screen** — i.e. after the user's first **real** walk finishes — gated by a `UserDefaults` flag `onboardingVocabCardShown` (unset ⇒ show once, then set). **Remove the stale `TODO(#181)`** in the MIC step (replace with a one-line comment pointing at the notes-screen hook). The flag is a UX mirror of the core `_seeds` marker; **core stays authoritative for idempotency** (a shown-but-skipped card leaves core cold, exactly today's behavior).
- [ ] **Free-form terms on Done — per-term catch-and-continue** (`// sac:`): the FFI `addVocabularyTerm` **throws** at `Full`/`Empty`/`TooLong` (`crates/ffi/src/vocabulary.rs:36–48`), so a naive `for t in freeform { try engine.addVocabularyTerm(t) }` **aborts the whole batch** on the first bad/over-cap term. Instead loop with a **per-term `do/catch`**, incrementing a `skippedCount` on failure and continuing; surface the skipped count in the card's confirmation (e.g. "Added N, skipped M"). `seedVocabulary` itself already tallies internally (`SeedReport`) — this only concerns the free-form additions written one-at-a-time through the CRUD method.
...
2. Seeding is **idempotent** per `trade:version` (second call = no-op, `already_seeded:true`, no save) and **deletion is durable** (a removed seeded term is not resurrected by re-seed) — WE-B — **including across a reflection cycle**: `reflect()` carries the `_seeds` marker forward so `is_pack_seeded` stays true post-reflection and a re-seed still short-circuits.
...
6. Internal `_`-prefixed sections are excluded from render/word_count/clamp/prune **and carried forward through `reflect()`** (the fifth site); the empty-wipe guard + `memory_block` compare **non-internal** sections; `churn_between` excludes internal; `UpdateMemoryTool` **rejects** `_`-prefixed section writes; **no existing (non-`_`) section behavior changes** (Task 1 regression + reflection pins green).
...
- A `MemoryEntry` **schema change**, a new `FactSource` variant, or a per-term "seeded" flag; a **`Pinned`/protected tier** for seeds (Plan 10 D3 escalation — deferred, Open Question 3).
- **Auto-harvest** of proper nouns from live extraction (Plan 10 D9 seam — `Inferred` source ready; detection not built).
...
- **Risk — reflection silently erases the marker (the Rev 2 BLOCKER).** `reflect()` rebuilds memory from model-echoed sections only; without the fifth-site carry-over, `_seeds` is dropped every reflection and user-deleted seeds resurrect on the next re-seed. Mitigation: the carry-over copies internal sections from `current` before the guard, pinned by a RED reflection test (marker preserved, never in the prompt, churn unaffected). Rollback: same as above — reverting Task 1 removes both the marker and the carry-over together.
...
3. **Protected `Pinned` tier (dam).** If device testing shows seeds eroding under cap/age pressure (they're `Stated`, so prunable), promote a protected tier (Plan 10 D3). — **ship `Stated` + measure first.**
